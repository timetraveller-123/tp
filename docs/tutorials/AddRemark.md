...
This command allows users of the PharmHub application to add optional remarks to people in their pharmHub and edit it if required. The command should have the following format:
...
/**
 * Changes the remark of an existing person in the pharmHub.
 */
...
Now that we have our `RemarkCommand` ready to be executed, we need to update `PharmHubParser#parseCommand()` to recognize the `remark` keyword. Add the new command to the `switch` block by creating a new `case` that returns a new instance of `RemarkCommand`.

You can refer to the changes in this [diff](https://github.com/se-edu/pharmHub-level3/commit/35eb7286f18a029d39cb7a29df8f172a001e4fd8#diff-399c284cb892c20b7c04a69116fcff6ccc0666c5230a1db8e4a9145def8fa4ee).
...
Your code should look something like [this](https://github.com/se-edu/pharmHub-level3/commit/dc6d5139d08f6403da0ec624ea32bd79a2ae0cbf#diff-a8e35af8f9c251525063fae36c9852922a7e7195763018eacec60f3a4d87c594) after you are done.

### Parse user input

Now let’s move on to writing a parser that will extract the index and remark from the input provided by the user.

Create a `RemarkCommandParser` class in the `seedu.pharmhub.logic.parser` package. The class must extend the `Parser` interface.
...
:information_source: Don’t forget to update `PharmHubParser` to use our new `RemarkCommandParser`!

</div>

If you are stuck, check out the sample
[here](https://github.com/se-edu/pharmHub-level3/commit/dc6d5139d08f6403da0ec624ea32bd79a2ae0cbf#diff-8bf239e8e9529369b577701303ddd96af93178b4ed6735f91c2d8488b20c6b4a).
...
Create a new `Remark` in `seedu.pharmhub.model.person`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.

A copy-paste and search-replace later, you should have something like [this](https://github.com/se-edu/pharmHub-level3/commit/4516e099699baa9e2d51801bd26f016d812dedcc#diff-41bb13c581e280c686198251ad6cc337cd5e27032772f06ed9bf7f1440995ece). Note how `Remark` has no constrains and thus does not require input
validation.
...
Simply add the following to [`seedu.pharmhub.ui.PersonCard`](https://github.com/se-edu/pharmHub-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-639834f1e05afe2276a86372adf0fe5f69314642c2d93cfa543d614ce5a76688).
...
Then insert the following into [`main/resources/view/PersonListCard.fxml`](https://github.com/se-edu/pharmHub-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-d44c4f51c24f6253c277a2bb9bc440b8064d9c15ad7cb7ceda280bca032efce9).
...
Refer to [this commit](https://github.com/se-edu/pharmHub-level3/commit/ce998c37e65b92d35c91d28c7822cd139c2c0a5c) and check that you have got everything in order!


## Updating Storage

PharmHub stores data by serializing `JsonAdaptedPerson` into `json` with the help of an external library — Jackson. Let’s update `JsonAdaptedPerson` to work with our new `Person`!

While the changes to code may be minimal, the test data will have to be updated as well.

<div markdown="span" class="alert alert-warning">

:exclamation: You must delete PharmHub’s storage file located at `/data/pharmHub.json` before running it! Not doing so will cause PharmHub to default to an empty pharmHub!

</div>

Check out [this commit](https://github.com/se-edu/pharmHub-level3/commit/556cbd0e03ff224d7a68afba171ad2eb0ce56bbf)
...
Just add [this one line of code!](https://github.com/se-edu/pharmHub-level3/commit/5b98fee11b6b3f5749b6b943c4f3bd3aa049b692)
...
You should end up with a test that looks something like [this](https://github.com/se-edu/pharmHub-level3/commit/fac8f3fd855d55831ca0cc73313b5943d49d4d6e#diff-ff58f7c10338b34f76645df49b71ecb2bafaf7611b20e7ff59ebc98475538a01R36-R49).

## Conclusion

This concludes the tutorial for adding a new `Command` to PharmHub.
